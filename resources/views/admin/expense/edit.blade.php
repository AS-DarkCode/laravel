...
@section('content')
    <div class="appHeader bg-gradient-primary text-light border-0 d-flex align-items-center px-3" style="height: 60px; background: linear-gradient(135deg, #007bff, #0056b3);">
        ...
        <div class="pageTitle flex-grow-1 text-center">
            <span class="fw-bold" style="font-size: 20px; letter-spacing: 0.5px;">Edit Expense</span>
        </div>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('expense.update', $expense->id) }}">
                        ...
                        <div class="form-group basic mb-3">
                            ...
                            <input type="text" class="form-control shadow-sm rounded-3 @error('itemname') is-invalid @enderror" id="itemname" name="itemname" placeholder="Enter item name" value="{{ old('itemname', $expense->itemname) }}" required>
                            @error('itemname')
                        ...
                        <div class="form-group basic mb-3">
                            ...
                            <input type="date" class="form-control shadow-sm rounded-3 @error('date') is-invalid @enderror" id="date" name="date" value="{{ old('date', $expense->date->format('Y-m-d')) }}" required>
                            @error('date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group basic mb-3">
                            ...
                            <input type="text" class="form-control shadow-sm rounded-3 @error('location') is-invalid @enderror" id="location" name="location" placeholder="Enter location" value="{{ old('location', $expense->location) }}" required>
                            @error('location')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group basic mb-3">
                            ...
                            <select class="form-control shadow-sm rounded-3 custom-select @error('userid') is-invalid @enderror" id="userid" name="userid">
                                <option value="" {{ old('userid', $expense->userid) == '' ? 'selected' : '' }}>Default ({{ Auth::user()->name }})</option>
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" {{ old('userid', $expense->userid) == $user->id ? 'selected' : '' }}>{{ $user->name }} ({{ ucfirst($user->role) }})</option>
                                @endforeach
                            </select>
                            @error('userid')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="form-group basic mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" class="form-control shadow-sm rounded-end-3 @error('amount') is-invalid @enderror" id="amount" name="amount" placeholder="Enter amount" value="{{ old('amount', $expense->amount) }}" required>
                            </div>
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group basic mt-4">
                            <button type="submit" class="btn btn-primary btn-block rounded-pill py-3 fw-semibold shadow-sm" style="transition: transform 0.3s ease;">Update Expense</button>
                        </div>
                    </form>
                </div>
            </div>
