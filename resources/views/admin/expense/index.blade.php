...
@section('content')
    <div class="appHeader bg-gradient-primary text-light border-0 d-flex align-items-center px-3" style="height: 60px; background: linear-gradient(135deg, #007bff, #0056b3);">
        ...
        <div class="pageTitle flex-grow-1 text-center">
            <span class="fw-bold" style="font-size: 20px; letter-spacing: 0.5px;">Manage Expenses</span>
        </div>
        <div class="right d-flex align-items-center gap-2">
            <a href="{{ route('expense.create') }}" class="headerButton d-flex align-items-center justify-content-center" style="width: 40px; height: 40px; background: rgba(255,255,255,0.2); border-radius: 50%; transition: background 0.3s ease;" title="Add Expense">
            ...
            @if ($payments->isEmpty())
                <div class="card border-0 shadow-sm rounded-4 text-center">
                    <div class="card-body py-5">
                        ...
                        <h5 class="mt-3 text-muted fw-semibold">No Expenses Found</h5>
                        <p class="text-muted mb-0 small">Start by adding an expense.</p>
                    </div>
                </div>
            @else
                    ...
                    <table class="table table-hover border-0 rounded-3">
                        <thead class="bg-light">
                            <tr>
                                ...
                                <th scope="col" class="py-3 text-muted small fw-semibold text-start" style="font-size: 0.85rem;">User</th>
                                <th scope="col" class="py-3 text-muted small fw-semibold text-start" style="font-size: 0.85rem;">Item</th>
                                <th scope="col" class="py-3 text-muted small fw-semibold text-start" style="font-size: 0.85rem;">Location</th>
                        ...
                        <tbody>
                            @foreach ($payments as $payment)
                                <tr class="align-middle">
                                    ...
                                    <td class="py-3 text-start">
                                        <span class="text-muted small" style="font-size: 0.85rem;">{{ $payment['location'] ?? 'N/A' }}</span>
                                    </td>
                                    <td class="py-3 text-start">
                                    ...
                                    <td class="py-3 pe-3 text-end">
                                        <div class="d-flex justify-content-end gap-2">
                                            <a href="{{ route('expense.edit', $payment['id']) }}" class="btn btn-sm btn-outline-warning rounded-pill px-2 py-1" style="font-size: 0.75rem;">
                                                <ion-icon name="pencil-outline" style="font-size: 14px; vertical-align: middle;"></ion-icon>
                                            </a>
                                            <button type="button" class="btn btn-sm btn-outline-danger rounded-pill px-2 py-1" data-bs-toggle="modal" data-bs-target="#deleteModal{{ $payment['id'] }}" style="font-size: 0.85rem;">
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <div class="modal-content border-0 rounded-4 shadow-sm">
                                                        ...
                                                        <div class="modal-body text-center">
                                                            ...
                                                            <p class="mt-3 mb-0 fw-semibold">Are you sure you want to delete this expense?</p>
                                                        ...
                                                        <div class="modal-footer border-0 justify-content-center">
                                                            ...
                                                            <form action="{{ route('expense.destroy', $payment['id']) }}" method="POST" style="display:inline;" onsubmit="return confirm('Are you absolutely sure?');">
                                                                @csrf
                                                                @method('DELETE')
