...
@section('content')
    ...
    <div class="appHeader bg-gradient-primary text-light border-0 d-flex align-items-center px-3" style="height: 60px; background: linear-gradient(135deg, #007bff, #0056b3);">
        ...
        <div class="pageTitle flex-grow-1 text-center">
            <span class="fw-bold" style="font-size: 20px; letter-spacing: 0.5px;">Edit Payment</span>
        </div>
    ...
    <div id="appCapsule" class="pb-5">
        <div class="section mt-3 px-3">
            ...
            <div class="card border-0 shadow-lg rounded-4 bg-gradient-light" style="background: linear-gradient(135deg, #f5f7fa, #e4e7eb);">
                <div class="card-body">
                    <form method="POST" action="{{ route('send.update', $payment->id) }}">
                        @csrf
                        @method('POST')

                        <!-- Recipient (User) -->
                        <div class="form-group basic mb-3">
                            <label class="label fw-semibold text-dark mb-2" for="userid">Recipient (User)</label>
                        ...
                        <div class="form-group basic mb-3">
                            ...
                            <textarea class="form-control shadow-sm rounded-3 @error('breif') is-invalid @enderror" id="breif" name="breif" placeholder="Enter payment brief" rows="3" required>{{ old('breif', $payment->breif) }}</textarea>
                            @error('breif')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group basic mb-3">
                            ...
                            <select class="form-control shadow-sm rounded-3 custom-select @error('paymenttype') is-invalid @enderror" id="paymenttype" name="paymenttype" required>
                                ...
                                <option value="cash" {{ old('paymenttype', $payment->paymenttype) == 'cash' ? 'selected' : '' }}>Cash</option>
                                <option value="online" {{ old('paymenttype', $payment->paymenttype) == 'online' ? 'selected' : '' }}>Online</option>
                            </select>
                            @error('paymenttype')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group basic mb-3">
                            ...
                            <input type="date" class="form-control shadow-sm rounded-3 @error('transationdate') is-invalid @enderror" id="transationdate" name="transationdate" value="{{ old('transationdate', $payment->transationdate->format('Y-m-d')) }}" required>
